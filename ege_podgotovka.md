...
### Задание номер 13 - IPадреса 
- импорт либы: `from ipaddress import *`
- создание сети: `net = ip_network('134.181.67.112/255.0.0.0')`

3. не всегда можно решить в питоне так как библиотека шарит за реальные айпи адреса
- теория кратко
```адр
адрес узла - изменяемая часть ip адреса
адрес сети - неизменяемая часть  ip addresa 
маска - определяем адрес сети или адрес узла при наличии одного из предыдущих пунктов(получаем через логическое 'и'
```   
